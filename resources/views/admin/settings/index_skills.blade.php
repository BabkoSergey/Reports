...
@section('htmlheader_title') {{__('Skills Settings')}} @endsection
        ...
        <div class="col-md-12 jq_start_main">
            ...
            <div class="nav-tabs-custom js-main-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="#skills_cat" data-toggle="tab">{{__('Category parameters')}}</a></li>                                    
<!--                    <li><a href="#mail" data-toggle="tab">{{__('Mail')}}</a></li>                                -->
                </ul>
                
                <div class="tab-content">
                    <div class="tab-pane active" id="skills_cat">                         
                        {!! Form::model($settings['skills_cat'], ['method' => 'PATCH','route' => ['settings.update', 'skills_cat'], 'class' => 'form-horizontal', 'id'=>'skills_cat_form']) !!}
                            <div class="box-header">
                                <a class="btn btn-success margin-l-10 jq_settings-btn-add"> {{ __('Add New') }}</a>                                
                            </div>
                        
                            <div class="box-body jq_setting-box-skills_cat">                                 
                                @foreach($settings['skills_cat'] as $code => $val)   
                                    @include('admin.settings.row_key_val', ['codePrefix' => 'skills_cat'])
                                @endforeach    
                            </div><!-- /.box-body -->            
                            
                            <div class="box-footer">
                                <a class="btn btn-default" role="button" href="{{ route('settings.skills.index') }}">{{ __('Cancel') }}</a>
...
@push('scripts')    
    <script>
        $(document).ready(function () {
            ...
            $(document).on('submit','#skills_cat_form',function (e){
                    e.preventDefault();                                         
                    $('.alert').remove();
